...
from crud.auth import (
    ...
    create_reset_otp, verify_reset_otp, reset_user_password,  # ← new
...
class ResetPasswordRequest(BaseModel):
    reset_token:  str   # short-lived JWT returned by /verify-reset-otp
    new_password: str
...
# ── /forgot-password ──────────────────────────────────────────────────────────
# Step 1: user submits their email → we send a reset OTP.
# Always returns 200 so we don't reveal whether the account exists.
...
def forgot_password(req: ForgotPasswordRequest):
    ...
    if user:
        otp = generate_otp()
        create_reset_otp(str(user.id), otp)          # stored separately in memory
        try:
            send_otp_email(user.email, otp, user.first_name)
        except Exception as e:
            logger.error("forgot-password email error: %s", e)
    # Always the same response — don't leak account existence
    return {"message": "If that email exists, a reset code has been sent."}


# ── /verify-reset-otp ─────────────────────────────────────────────────────────
# Step 2: user submits { email, otp_code } → we verify and hand back a
# short-lived reset_token JWT (15 min, type="password_reset").

@app.post("/api/auth/verify-reset-otp")
def verify_reset_otp_route(req: VerifyResetOtpRequest):
    user = get_user_by_email(req.email.strip().lower())
    # Use same error for "no account" and "wrong OTP" to avoid enumeration
    if not user:
        return JSONResponse(status_code=400, content={"message": "Invalid or expired code."})

    ok = verify_reset_otp(str(user.id), req.otp_code.strip())
    if not ok:
        return JSONResponse(status_code=400, content={"message": "Invalid or expired code."})

    # Issue a 15-minute single-purpose JWT so /reset-password is authenticated
...
# ── /reset-password ───────────────────────────────────────────────────────────
# Step 3: user submits { reset_token, new_password } → we save the new hash
# and log them in immediately by returning a fresh access_token.

@app.post("/api/auth/reset-password")
def reset_password_route(req: ResetPasswordRequest, response: Response):
    payload = decode_token(req.reset_token)
    if not payload or payload.get("type") != "password_reset":
        return JSONResponse(status_code=400, content={"message": "Reset link is invalid or has expired."})

    user_id = payload.get("sub")
    if not user_id:
        return JSONResponse(status_code=400, content={"message": "Reset link is invalid."})
...
async def chat(
    ...
    try:
        ...
        if req.file_content:
            if getattr(req, "file_type", None) == "pdf":
                try:
                    ...
                    logger.info("PDF '%s' extracted: %d pages, %d chars", req.file_name, pdf_page_count, len(resolved_content))
                except Exception as pdf_err:
                    logger.warning("PDF extraction failed: %s", pdf_err)
                    return JSONResponse(status_code=422, content={"response": f"⚠️ Could not read this PDF: {pdf_err}"})
            else:
        ...
        if resolved_content:
            ...
            tail    = "\n\n[... content truncated due to length ...]" if len(resolved_content) > 14_000 else ""
...
async def run_code(req: CodeRequest):
    ...
    if lang not in LANG_CONFIGS:
        return JSONResponse(status_code=400, content={"error": f"Language '{lang}' is not supported."})

    cfg = LANG_CONFIGS[lang]

    if cfg.get("java"):
        if not shutil.which("javac") or not shutil.which("java"):
            return JSONResponse(status_code=400, content={"error": "Java not installed on server."})
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with open(src, "w", encoding="utf-8") as f:
                    f.write(req.code)
                comp = subprocess.run(["javac", src], capture_output=True, text=True, timeout=20, cwd=tmpdir)
                if comp.returncode != 0:
                    return {"error": comp.stderr}
                result = subprocess.run(["java", "-cp", tmpdir, classname], capture_output=True, text=True, timeout=10)
    ...
    try:
        ...
        cmd = cmd_fn(tmp_path)
        if cmd is None:
            return JSONResponse(status_code=400, content={"error": f"Runtime for '{lang}' not installed."})
    ...
    try:
        if filename.endswith(".pdf"):
            ...
            try:
                ...
                return JSONResponse(status_code=400, content={"error": "python-docx not installed."})
